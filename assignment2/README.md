...
# Or if using VS Code, the project compiles automatically
```
...
- CSV parsing (valid and malformed data)
- Aggregation accuracy
- Case-insensitive filtering

### Running Tests
```bash
java -cp bin utils.SalesDataAnalyzerTest
```

Tests use simple assertions - **no external testing frameworks required**. Each test validates:
- Expected vs actual values
- Map sizes and contents
- List sizes and ordering
...
- Immutable `SalesRecord` class ensures thread-safety
- Defensive CSV parsing skips malformed lines with error logging
- Case-insensitive region filtering for robustness
- No external dependencies - runs with standard JDK only

## Error Handling
- Malformed CSV lines are skipped with error messages logged to stderr
- Empty files return empty result sets
